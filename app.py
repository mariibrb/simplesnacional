...
def extrair_dados(conteudo, chaves_vistas, chaves_canc, cnpj_cliente, uf_cliente):
    ...
    try:
        ...
        # Trava de Segurança por CNPJ (Limpo)
...
def main():
    ...
    if st.button("🚀 Iniciar Auditoria") and cnpj_cli:
        ...
        if registros:
            ...
            aliq_ef = (((rbt12 * aliq_nom) - ded) / rbt12).quantize(Decimal("0.0000000000000001"), ROUND_HALF_UP) if rbt12 > 0 else aliq_nom
            ...
            for cfop in df['CFOP'].unique():
                ...
                # Subtração Real de Base ST da Base Normal
                ...
                resumo_cfop.append({"CFOP": cfop, "Tipo": tipo, "ST": is_st, "V. Bruto": v_bruto, "Base Líquida": v_liq, "Alíquota": f"{aliq*100:.13f}%", "Imposto": imp})
            ...
            st.error(f"❌ Nenhuma nota encontrada para o CNPJ {cnpj_cli}. Verifique se o CNPJ das notas é o mesmo digitado.")
