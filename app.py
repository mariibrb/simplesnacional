...
Foco: Conciliação Analítica com Precisão PGDAS e Varredura de DIFAL
"""
...
def extrair_dados_xml(conteudo, chaves_vistas, chaves_canc, cnpj_cliente, uf_cliente):
    ...
    try:
        ...
        emit_node = inf.find(f"{ns}emit")
        emit_cnpj = limpar_formato(emit_node.find(f"{ns}CNPJ").text)
        emit_uf = emit_node.find(f"{ns}enderEmit/{ns}UF").text
        
        dest_node = inf.find(f"{ns}dest")
        dest_cnpj = limpar_formato(dest_node.find(f"{ns}CNPJ").text) if dest_node is not None and dest_node.find(f"{ns}CNPJ") is not None else ""
        ...
        # Lógica de Faturamento (Saída do Cliente)
        ...
        # Lógica de Devolução e DIFAL (Entrada no Cliente)
...
def main():
    st.title("🛡️ Sentinela - Memorial Analítico Universal")
    ...
    if st.button("🚀 Gerar Auditoria") and cnpj_cli:
        ...
        if registros:
            ...
            # Consolidação por CFOP com Subtração Real
            resumo_cfop = []
            for cfop in df['CFOP'].unique():
                sub_df = df[df['CFOP'] == cfop]
                tipo = sub_df['Tipo'].iloc[0]
                is_st = sub_df['ST'].any()
                v_bruto = sub_df['Valor'].sum()
                
                # Regra: Abater o valor de ST da base normal se for saída
            ...
            if lista_difal:
                st.markdown(f'<div class="difal-box">⚠️ <b>DIFAL ENTRADA:</b> R$ {sum(d["Valor Base"] for d in lista_difal):,.2f} detectado em compras interestaduais.</div>', unsafe_allow_html=True)

            # MEMORIAL
            st.markdown("### 📝 Memorial de Cálculo Analítico")
            ...
            st.error(f"❌ Nenhuma nota autorizada para o CNPJ {cnpj_cli} foi encontrada. Verifique os arquivos.")
